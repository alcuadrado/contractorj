 ...
 * <ul>
 *   <li>
 *       <p>Absolute and relative {@link #get() </code><i>get</i><code>} and {@link #put(byte)
 *       </code><i>put</i><code>} methods that read and write single bytes;
 *   <li>
 *       <p>Relative {@link #get(byte[]) </code><i>bulk get</i><code>} methods that transfer
 *       contiguous sequences of bytes from this buffer into an array;
 *   <li>
 *       <p>Relative {@link #put(byte[]) </code><i>bulk put</i><code>} methods that transfer
 *       contiguous sequences of bytes from a byte array or some other byte buffer into this buffer;
 *   <li>
 *       <p>Absolute and relative {@link #getChar() </code><i>get</i><code>} and {@link
 *       #putChar(char) </code><i>put</i><code>} methods that read and write values of other
 *       primitive types, translating them to and from sequences of bytes in a particular byte
 *       order;
 *   <li>
 *       <p>Methods for creating <i><a href="#views">view buffers</a></i>, which allow a byte buffer
 *       to be viewed as a buffer containing values of some other primitive type; and
 *   <li>
 *       <p>Methods for {@link #compact </code>compacting<code>}, {@link #duplicate
 *       </code>duplicating<code>}, and {@link #slice </code>slicing<code>} a byte buffer.
 * </ul>
 *
 * <p>Byte buffers can be created either by {@link #allocate </code><i>allocation</i><code>}, which
 * allocates space for the buffer's
 *
 * <p>content, or by {@link #wrap(byte[]) </code><i>wrapping</i><code>} an existing byte array into
 * a buffer.
 *
 * <p><a name="direct">
 *
 ...
 *
 * <p>A byte buffer is either <i>direct</i> or <i>non-direct</i>. Given a direct byte buffer, the
 * Java virtual machine will make a best effort to perform native I/O operations directly upon it.
 * That is, it will attempt to avoid copying the buffer's content to (or from) an intermediate
 * buffer before (or after) each invocation of one of the underlying operating system's native I/O
 * operations.
 *
 * <p>A direct byte buffer may be created by invoking the {@link #allocateDirect(int)
 * allocateDirect} factory method of this class. The buffers returned by this method typically have
 * somewhat higher allocation and deallocation costs than non-direct buffers. The contents of direct
 * buffers may reside outside of the normal garbage-collected heap, and so their impact upon the
 * memory footprint of an application might not be obvious. It is therefore recommended that direct
 * buffers be allocated primarily for large, long-lived buffers that are subject to the underlying
 * system's native I/O operations. In general it is best to allocate direct buffers only when they
 * yield a measureable gain in program performance.
 *
 * <p>A direct byte buffer may also be created by {@link java.nio.channels.FileChannel#map
 * </code>mapping<code>} a region of a file directly into memory. An implementation of the Java
 * platform may optionally support the creation of direct byte buffers from native code via JNI. If
 * an instance of one of these kinds of buffers refers to an inaccessible region of memory then an
 * attempt to access that region will not change the buffer's content and will cause an unspecified
 * exception to be thrown either at the time of the access or at some later time.
 *
 * <p>Whether a byte buffer is direct or non-direct may be determined by invoking its {@link
 * #isDirect isDirect} method. This method is provided so that explicit buffer management can be
 ...
 *
 * <p>This class defines methods for reading and writing values of all other primitive types, except
 * <tt>boolean</tt>. Primitive values are translated to (or from) sequences of bytes according to
 * the buffer's current byte order, which may be retrieved and modified via the {@link #order order}
 * methods. Specific byte orders are represented by instances of the {@link ByteOrder} class. The
 * initial order of a byte buffer is always {@link ByteOrder#BIG_ENDIAN BIG_ENDIAN}.
 *
 * <p>For access to heterogeneous binary data, that is, sequences of values of different types, this
 ...
 * </blockquote>
 *
 * <p>Corresponding methods are defined for the types <tt>char</tt>, <tt>short</tt>, <tt>int</tt>,
 * <tt>long</tt>, and <tt>double</tt>. The index parameters of the absolute <i>get</i> and
 * <i>put</i> methods are in terms of bytes rather than of the type being read or written.
 *
 * <p><a name="views">
 *
 * <p>For access to homogeneous binary data, that is, sequences of values of the same type, this
 * class defines methods that can create <i>views</i> of a given byte buffer. A <i>view buffer</i>
 * is simply another buffer whose content is backed by the byte buffer. Changes to the byte buffer's
 * content will be visible in the view buffer, and vice versa; the two buffers' position, limit, and
 * mark values are independent. The {@link #asFloatBuffer() asFloatBuffer} method, for example,
 * creates an instance of the {@link FloatBuffer} class that is backed by the byte buffer upon which
 * the method is invoked. Corresponding view-creation methods are defined for the types
 * <tt>char</tt>, <tt>short</tt>, <tt>int</tt>, <tt>long</tt>, and <tt>double</tt>.
 *
 * <p>View buffers have three important advantages over the families of type-specific <i>get</i> and
 * <i>put</i> methods described above:
 *
 * <ul>
 *   <li>
 *       <p>A view buffer is indexed not in terms of bytes but rather in terms of the type-specific
 *       size of its values;
 *   <li>
 *       <p>A view buffer provides relative bulk <i>get</i> and <i>put</i> methods that can transfer
 *       contiguous sequences of values between a buffer and an array or some other buffer of the
 *       same type; and
 *   <li>
 *       <p>A view buffer is potentially much more efficient because it will be direct if, and only
 *       if, its backing byte buffer is direct.
 * </ul>
 *
 * <p>The byte order of a view buffer is fixed to be that of its byte buffer at the time that the
 * view is created.
 *
 * <h4>Invocation chaining </h4>
 *
 * <p>Methods in this class that do not otherwise have a value to return are specified to return the
 * buffer upon which they are invoked. This allows method invocations to be chained.
 *
 ...
 *
 * <pre>
 * bb.putInt(0xCAFEBABE).putShort(3).putShort(45);</pre>
 *
 * </blockquote>
 *
...
public abstract class ByteBuffer extends Buffer implements Comparable<ByteBuffer> {
   ...
   *
   * <p>The new buffer's position will be zero, its limit will be its capacity, its mark will be
   * undefined, and each of its elements will be initialized to zero. Whether or not it has a {@link
   * #hasArray </code>backing array<code>} is unspecified.
   *
   ...
   *
   * <p>The new buffer's position will be zero, its limit will be its capacity, its mark will be
   * undefined, and each of its elements will be initialized to zero. It will have a {@link #array
   * </code>backing array<code>}, and its {@link #arrayOffset </code>array offset<code>} will be
   * zero.
   *
   * @param capacity The new buffer's capacity, in bytes
   * @return The new byte buffer
   ...
   *
   * <p>The new buffer will be backed by the given byte array; that is, modifications to the buffer
   * will cause the array to be modified and vice versa. The new buffer's capacity will be
   * <tt>array.length</tt>, its position will be <tt>offset</tt>, its limit will be <tt>offset +
   * length</tt>, and its mark will be undefined. Its {@link #array </code>backing array<code>} will
   * be the given array, and its {@link #arrayOffset </code>array offset<code>} will be zero.
   *
   ...
   *
   * <p>The new buffer will be backed by the given byte array; that is, modifications to the buffer
   * will cause the array to be modified and vice versa. The new buffer's capacity and limit will be
   * <tt>array.length</tt>, its position will be zero, and its mark will be undefined. Its {@link
   * #array </code>backing array<code>} will be the given array, and its {@link #arrayOffset
   * </code>array offset<code>} will be zero.
   *
   ...
   *
   * <p>The content of the new buffer will start at this buffer's current position. Changes to this
   ...
   *
   * <p>If this method returns <tt>true</tt> then the {@link #array() array} and {@link
   * #arrayOffset() arrayOffset} methods may safely be invoked.
   *
  ...
  /**
   * Returns the offset within this buffer's backing array of the first element of the
   * buffer&nbsp;&nbsp;<i>(optional operation)</i>.
   *
   * <p>If this buffer is backed by an array then buffer position <i>p</i> corresponds to array
   * index <i>p</i>&nbsp;+&nbsp;<tt>arrayOffset()</tt>.
   *
   * <p>Invoke the {@link #hasArray hasArray} method before invoking this method in order to ensure
   * that this buffer has an accessible backing array.
   *
   ...
   *
   * <p>The hash code of a byte buffer depends only upon its remaining elements; that is, upon the
   * elements from <tt>position()</tt> up to, and including, the element at
   * <tt>limit()</tt>&nbsp;-&nbsp;<tt>1</tt>.
   *
   * <p>Because buffer hash codes are content-dependent, it is inadvisable to use buffers as keys in
   ...
   * </ol>
   *
   * <p>A byte buffer is not equal to any other type of object.
   *
   ...
   *
   * <p>Two byte buffers are compared by comparing their sequences of remaining elements
   * lexicographically, without regard to the starting position of each sequence within its
   * corresponding buffer.
   *
   * <p>Pairs of {@code byte} elements are compared as if by invoking {@link
  ...
  boolean
      bigEndian // package-private
           = true;
  boolean
      nativeByteOrder // package-private
           = (Bits.byteOrder() == ByteOrder.BIG_ENDIAN);

  /**
   * Retrieves this buffer's byte order.
   *
   * <p>The byte order is used when reading or writing multibyte values, and when creating buffers
   * that are views of this byte buffer. The order of a newly-created byte buffer is always {@link
   ...
   *
   * <p>The new buffer's position will be zero, its capacity and its limit will be the number of
   * bytes remaining in this buffer divided by two, and its mark will be undefined. The new buffer
   * will be direct if, and only if, this buffer is direct, and it will be read-only if, and only
   ...
   *
   * <p>The new buffer's position will be zero, its capacity and its limit will be the number of
   * bytes remaining in this buffer divided by four, and its mark will be undefined. The new buffer
   * will be direct if, and only if, this buffer is direct, and it will be read-only if, and only
   * if, this buffer is read-only.
   *
   ...
   *
   * <p>The new buffer's position will be zero, its capacity and its limit will be the number of
   * bytes remaining in this buffer divided by eight, and its mark will be undefined. The new buffer
   * will be direct if, and only if, this buffer is direct, and it will be read-only if, and only
   * if, this buffer is read-only.
   *
