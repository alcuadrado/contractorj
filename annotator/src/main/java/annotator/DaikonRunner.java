...
public class DaikonRunner {
    ...
    public String readDaikonResults() {
        ...
        try {
            ...
            if (CACHED_RESULTS_FILE.exists() && CACHED_RESULTS_FILE.lastModified()
                    > DAIKON_RESULTS_FILE.lastModified()) {

                return Files.toString(CACHED_RESULTS_FILE, StandardCharsets.UTF_8);
            }

            final String results = CommandsRunner.runtAndReturnOutput("java -cp '" + daikonJar.getAbsolutePath()
                    + "' daikon.PrintInvariants --format java " + DAIKON_RESULTS_FILE);
