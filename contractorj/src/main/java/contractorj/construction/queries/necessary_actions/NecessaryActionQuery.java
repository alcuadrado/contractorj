...
public abstract class NecessaryActionQuery extends Query {
  ...
  public String getName() {
    ...
    final String name =
        super.getName()
            ...
            + "testing"
            + NAME_PART_SEPARATOR
            + testedAction.getMethod().getJavaNameWithArgumentTypes();
  ...
  protected String getQueryCore() {
    ...
    return getStatePreconditionCall(testedAction).map(s -> s + "\n" + "\n" + "\n").orElse("")
        ...
        + "query_assertion_2:\n"
        + "assert "
        + (isTestingEnabledness() ? "" : "!")
        + getTestedActionReturnVariable().name
        + ";";
  }
  ...
  private Variable getTestedActionReturnVariable() {
    ...
    if (!testedAction.getStatePrecondition().isPresent()) {
      throw new IllegalStateException(
          testedAction + " has no state precondition so it's always enabled");
    }
