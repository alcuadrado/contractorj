...
JavaService.exe -install %SERVICE_NAME% %JDK_HOME_DIR%\jre\bin\%JDK_MODE%\jvm.dll -Djava.class.path=%LOCALCLASSPATH% -start Mail -params %JES_HOME_DIR% -stop Mail -method shutdown -out %JES_HOME_DIR%\jes-service.log -err %JES_HOME_DIR%\jes-service-error.log -current %JES_HOME_DIR%\bin
goto eof
