...
import examples.smtp_server.src.errors.NotFoundException;
...
public class SMTPSender implements Runnable {
                ...
                if( smtpDirectory.exists() && smtpDirectory.isDirectory() ) {
                    ...
                    for( int index = 0; index < numFiles; index++ ) {
                        try {
                            ...
                            log.error( "An error occured attempting to deliver an SMTP Message: " + throwable, (Exception) throwable);
    ...
    // ESTO ES UN MOCKEO - en boogie deberia hacer un havoc - para que el constructor no pise todo
    private EmailAddress get(int i){
    ...
    private void deliver( SMTPMessage message ) {
        ...
        for( int index = 0; index < numAddress; index++ ) {
            try {
                ...
                log.error( "Delivery failed for message from: " + message.getFromAddress().getAddress() + " to: " + address + " - " + throwable, (Exception) throwable);
                //failedAddress.addElement( toAddresses.get( index ) );
                failedAddress.addElement( get( index ) );
            }
        }
