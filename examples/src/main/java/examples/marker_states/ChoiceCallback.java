...
public class ChoiceCallback implements Callback, java.io.Serializable {
    ...
    public static boolean ChoiceCallback_pre(String prompt, String[] choices, int defaultChoice, boolean multipleSelectionsAllowed){
        if (prompt == null || prompt.length() == 0 ||
                choices == null || choices.length <= 0 ||
                defaultChoice < 0 || defaultChoice >= choices.length)
            return false;

        // corral fails to check this since there is a recursion bound
        for (int i = 0; i < choices.length; i++) {
            if (choices[i] == null || choices[i].length() == 0)
                return false;
        }
    ...
    public boolean getSelectedIndexes_pre(){
    ...
    public boolean setSelectedIndexes_pre(){
        return multipleSelectionsAllowed;
    }
